...
function getDb() {
  ...
  try {
    ...
    // The most common cause by far: the mounted db/ volume is owned by a
    // different uid than the container runs as (a lot of hosting platforms
    // — k8s PVCs, several PaaS — don't honor the image's baked-in
    // ownership). Say so instead of leaving a bare native SQLite error.
    err.message = `Could not open ${DB_PATH} (${err.message}). If this is a fresh deployment, check that the db/ volume is writable by the container's user.`;
    throw err;
  }
  ...
  db.exec(`
    ...
    -- Per-user third-party credentials for the optional sync integrations.
    -- These are SECRETS: they're written from the profile page and used
    -- server-side only — /api/settings never returns them to the browser,
    -- it only reports which ones are set. Everything here is optional; an
    -- empty credential simply means that integration is skipped.
